...
N = int(input())
Diego_collection = [int(i) for i in input().split()]
K = int(input())
Guests_collection = [int(i) for i in input().split()]
...
while K:
    if N == 0:
        for i in range(K):
            print(0)
        break
    for guest_card in Guests_collection:
        count = 0
        for card in set(Diego_collection):
            if card < guest_card:
                count += 1
        print(count)
        K -= 1
