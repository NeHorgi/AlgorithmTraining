...
Обитатели мира целыми городами отправились на восток, покинув родные улицы, и двигались до тех пор,
...
Во второй строке дано N чисел a ** i (0 ≤ a ** i ≤10 ** 9) — средняя цена проживания в городах с нулевого по (N - 1)-ый соответственно.
...
for city in a:
    ...
    low_coster = city
    for new_city in a[i:]:
        if new_city < low_coster:
            result.append(a.index(new_city))
            i += 1
            break
    if check == len(result):
        result.append(-1)
